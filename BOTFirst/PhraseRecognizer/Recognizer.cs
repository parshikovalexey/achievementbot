    ...
    public class Recognizer {
        public static Phrase RecognizePhrase(string inputText) {
            ...
            if (inputText != "" && action != "") {
                ...
                phrase.RecognitionResult += "Что?/Кому?/Каким?/Куда? — " + inputText + "." + ConstantValues.NewLine;
            }
            ...
            if (phrase.RecognitionResult == "") {
                ...
                if (date != "" && time.Trim(new Char[] { 'в' }) != "")
                    phrase.RecognitionResult += String.Format("Когда? — Дата: {0}. Время: {1}.{2}DateTime: {3}", date, time, ConstantValues.NewLine, dateTime);
                else if (date == "" && time.Trim(new Char[] { 'в' }) != "")
                    phrase.RecognitionResult += String.Format("Когда? — Дата: не указана, будет считаться, что сегодня. Время: {0}.{1}DateTime: {2}", time, ConstantValues.NewLine, dateTime);
                else if (date != "" && time.Trim(new Char[] { 'в' }) == "")
                    phrase.RecognitionResult += String.Format("Когда? — Дата: {0}. Время: не указано, будет использовано текущее.{1}DateTime: {2}", date, ConstantValues.NewLine, dateTime);
                else
                    phrase.RecognitionResult += "Когда? — дата не была указана либо не была распознана, будут использованы текущие дата и время." + ConstantValues.NewLine + "DateTime: " + dateTime;
                return phrase;
            }
